...
pub fn get_nvidia_smi_data() -> HardwareEnumerationResult<Vec<RpcGpu>> {
    let cmd = Cmd::new("timeout")
        .args(vec!["--kill-after=120s", "60s", "nvidia-smi", "--format=csv,noheader", "--query-gpu=name,serial,driver_version,vbios_version,inforom.image,memory.total,clocks.applications.gr,pci.bus_id,platform.chassis_serial_number,platform.slot_number,
