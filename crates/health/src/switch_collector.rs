...
//! Switch NMX-T metrics collector
//!
//! This module collects NVLink health metrics from NMX-T telemetry endpoints
//! on NVLink switches. It scrapes the `/xcset/nvlink_domain_telemetry` endpoint
//! and re-exposes key metrics:
//! - Effective BER (threshold: >1e-6 indicates attention)
//! - Symbol Errors (threshold: >0 indicates attention)
//! - Link Down counter (threshold: >0 indicates attention)
...
/// NMX-T endpoint for NVLink domain telemetry
...
/// Parse a single Prometheus format line
...
/// Scrape NMX-T metrics from a single switch
