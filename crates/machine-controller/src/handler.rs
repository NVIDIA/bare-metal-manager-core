    ...
    async fn attempt_state_transition(
        ...
        match &mh_state {
            ...
            ManagedHostState::RotatingHostUefi { uefi_setup_info } => {
                handle_rotating_host_uefi(ctx, mh_snapshot, uefi_setup_info.clone()).await
            }
...
///    [`host_uefi_rotation::host_uefi_current_candidates`]) rather than assuming the
///    empty/factory password, so a host at v1 rotating to v2 succeeds;
...
async fn handle_rotating_host_uefi(
    ctx: &mut StateHandlerContext<'_, MachineStateHandlerContextObjects>,
    state: &ManagedHostStateSnapshot,
    uefi_setup_info: UefiSetupInfo,
) -> Result<StateHandlerOutcome<ManagedHostState>, StateHandlerError> {
...
async fn set_rotating_host_uefi_password(
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            if host_uefi_rotation::host_uefi_rotation_force_requested(state) {
