...
| Field | Type | Default | Description |
| ------- | ------ | --------- | ------------- |
| `allow_instance_vf` | `bool` | `true` | Global instance-VF admission switch for creation and network updates. `false` rejects every VF. With DPF intercept topology, `true` additionally requires every requested VF ID to be explicitly selected by that topology. DPF without intercept topology preserves the historical boolean-only admission behavior. Pre-DPF admission additionally requires the VF to be present in the effective `hbn_reps` and `dpu_config.num_of_vfs` inventory. |
| `hbn_reps` | `Option<String>` | — | Comma-separated representors HBN is expected to use during DPU provisioning. For pre-DPF instance admission, `pf0vfN` entries and inclusive `pf0vfN-pf0vfM` ranges select tenant VFs, capped by `dpu_config.num_of_vfs`; other representors do not select tenant VFs. Omitted or empty values use HBN's VF0 through VF13 fallback. Malformed PF0 VF selectors cause instance creation and network updates to fail validation. DPF ignores this field. |
