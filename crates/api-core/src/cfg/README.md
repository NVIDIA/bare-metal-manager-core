...
| `bluefield_software_reprovision_enabled` | `bool` | `false` | Whether a BlueFieldSoftware change on a DPU's owning DPUDeployment counts as drift that needs reprovisioning. While off, a BlueFieldSoftware-provisioned deployment is judged on its `DPUFlavor` alone during the reprovision scan, so publishing new BlueFieldSoftware does not by itself queue the fleet. BFB-provisioned deployments are unaffected. This governs the reprovision scan only: deciding whether a DPUService rollout may reach a DPU always considers BlueFieldSoftware, whatever this is set to, because new services must never land on a DPU whose OS is about to be replaced. |
