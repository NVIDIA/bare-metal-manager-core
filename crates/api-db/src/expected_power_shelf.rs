...
/// is set, it matches by ID. Otherwise it matches by bmc_mac_address.
pub async fn update(
    txn: &mut PgConnection,
    power_shelf: &ExpectedPowerShelf,
) -> DatabaseResult<()> {
    let set_clause = "SET bmc_username=$1, bmc_password=$2, serial_number=$3, ip_address=$4, metadata_name=$5, metadata_description=$6, metadata_labels=$7, rack_id=$8";

    let (result, error_id) = if let Some(id) = power_shelf.expected_power_shelf_id {
        let query =
            format!("UPDATE expected_power_shelves {set_clause} WHERE expected_power_shelf_id=$9");
        ...
        let query = format!("UPDATE expected_power_shelves {set_clause} WHERE bmc_mac_address=$9");
        let r = sqlx::query(&query)
            .bind(&power_shelf.bmc_username)
            .bind(&power_shelf.bmc_password)
            .bind(&power_shelf.serial_number)
            ...
            .await
            .map_err(|err| DatabaseError::query(&query, err))?;
        (r, power_shelf.bmc_mac_address.to_string())
    };
