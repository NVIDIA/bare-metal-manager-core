    ...
    config.rack_types = RackTypeConfig {
        rack_types: [
            ...
            (
                "NVL36".to_string(),
                RackTypeDefinition {
                    name: "NVL36".to_string(),
                    description: "36x1 GB200 NVL36 rack".to_string(),
...
async fn test_add_expected_rack_creates_rack_entry(pool: sqlx::PgPool) {
    ...
    // Verify the rack was also created in the racks table with the embedded definition.
    let rack = db::rack::get(&pool, rack_id).await.unwrap();
    let rack_type_def = rack
        .config
        .rack_type_definition
        .expect("should have rack_type_definition");
    assert_eq!(rack_type_def.name, "NVL72");
}
