...
#[crate::sqlx_test(fixtures("create_expected_switch"))]
...
async fn test_delete(pool: sqlx::PgPool) -> () {
    ...
    db::expected_switch::delete(switch.bmc_mac_address, &mut txn)
        .await
    ...
    get_expected_switch_1(&mut txn).await;
...
async fn test_delete_expected_switch_by_id(pool: sqlx::PgPool) {
    ...
    assert_eq!(
        ...
        format!("Expected switch with id {} not found", explicit_id)
    );
}
...
async fn test_get_expected_switch_by_id_not_found(pool: sqlx::PgPool) {
    ...
    assert_eq!(
        err.message().to_string(),
        format!("Expected switch with id {} not found", nonexistent_id)
    );
}
