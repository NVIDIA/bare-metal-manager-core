...
#[crate::sqlx_test]
async fn iterate_over_all_power_shelves(pool: sqlx::PgPool) -> sqlx::Result<()> {
    let env = create_test_env(pool.clone()).await;
    let power_shelf_ids = common::api_fixtures::site_explorer::new_power_shelfs(&env, 1)
        .await
        .unwrap();
    ...
    let expected_total_count = expected_iterations * power_shelf_ids.len() as f64;
    ...
    let count = power_shelf_handler.count.load(Ordering::SeqCst) as f64;
    assert!(
...
#[crate::sqlx_test]
async fn test_power_shelf_concurrent_controllers(
    pool: sqlx::PgPool,
) -> Result<(), Box<dyn std::error::Error>> {
    let env = create_test_env(pool.clone()).await;
    let power_shelf_ids = common::api_fixtures::site_explorer::new_power_shelfs(&env, 1).await?;
    ...
    // Verify that all power shelf IDs were processed
    for power_shelf_id in power_shelf_ids {
        let guard = power_shelf_handler.counts_per_id.lock().unwrap();
        let count = guard
            .get(&power_shelf_id.to_string())
        ...
        assert!(
            count > 0,
            "Power shelf ID should have been processed with concurrent controllers"
        );
    }
...
#[crate::sqlx_test]
async fn test_power_shelf_controller_lock_mechanism(
    pool: sqlx::PgPool,
) -> Result<(), Box<dyn std::error::Error>> {
    let env = create_test_env(pool.clone()).await;
    let power_shelf_ids = common::api_fixtures::site_explorer::new_power_shelfs(&env, 1).await?;
