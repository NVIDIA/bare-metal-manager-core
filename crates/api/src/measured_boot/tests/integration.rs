...
#[cfg(test)]
mod tests {
    use nico_api_db::measured_boot::journal;
    ...
    pub async fn test_measured_boot_integration(
        ...
        let dell_r750_profile =
            nico_api_db::measured_boot::profile::new(&mut txn, None, &dell_r750_attrs).await?;
        ...
        let beer_journal = nico_api_db::measured_boot::journal::get_latest_for_machine_id(
            &mut txn,
            beer_louisiana.machine_id,
        )
        .await?
        ...
        let lime_report = nico_api_db::measured_boot::report::new(
            &mut txn,
        ...
        let lime_journal = nico_api_db::measured_boot::journal::get_latest_for_machine_id(
            &mut txn,
            lime_coconut.machine_id,
        )
        .await?
        ...
        let slippery_profile =
            nico_api_db::measured_boot::profile::load_from_attrs(&mut txn, &dgx_h100_attrs)
                .await?
                .unwrap();

        let slippery_journal = nico_api_db::measured_boot::journal::get_latest_for_machine_id(
            &mut txn,
            slippery_lilac.machine_id,
        )
        .await?
        ...
        let cat_report = nico_api_db::measured_boot::report::new(
            &mut txn,
        ...
        let silly_journal = nico_api_db::measured_boot::journal::get_latest_for_machine_id(
            &mut txn,
            silly_salamander.machine_id,
        )
        .await?
        ...
        let cat_journal = nico_api_db::measured_boot::journal::get_latest_for_machine_id(
            &mut txn,
            cat_videos.machine_id,
        )
        .await?
        ...
        let bundle = nico_api_db::measured_boot::report::create_active_bundle(
            &mut txn,
            &princess_report,
            &Some(pcr_set),
        )
        ...
        assert_eq!(
            ...
            nico_api_db::measured_boot::journal::get_latest_for_machine_id(
                &mut txn,
                princess_network.machine_id
            )
            .await?
        ...
        let bundle = nico_api_db::measured_boot::report::create_revoked_bundle(
            &mut txn,
            &lime_report,
            &Some(pcr_set),
        )
        ...
        let bundle =
            nico_api_db::measured_boot::report::create_active_bundle(&mut txn, &cat_report, &None)
                .await?;
