...
use nico_api_db::measured_boot::bundle;
use nico_api_db::measured_boot::interface::bundle::{
...
pub async fn handle_create_measurement_bundle(
    ...
    let state = req.state();
    let bundle = nico_api_db::measured_boot::bundle::new(
        &mut txn,
        req.profile_id
...
pub async fn handle_delete_measurement_bundle(
    ...
    let bundle = match req.selector {
        ...
        Some(delete_measurement_bundle_request::Selector::BundleId(bundle_uuid)) => {
            nico_api_db::measured_boot::bundle::delete_for_id(&mut txn, bundle_uuid, false)
                .await
                .map_err(|e| CarbideError::Internal {
        ...
        Some(delete_measurement_bundle_request::Selector::BundleName(bundle_name)) => {
            nico_api_db::measured_boot::bundle::delete_for_name(&mut txn, bundle_name, false)
                .await
                .map_err(|e| CarbideError::Internal {
                    message: format!("deletion failed: {e}"),
                })?
...
pub async fn handle_rename_measurement_bundle(
    ...
    let bundle = match req.selector {
        ...
        Some(rename_measurement_bundle_request::Selector::BundleId(bundle_uuid)) => {
            nico_api_db::measured_boot::bundle::rename_for_id(
                &mut txn,
                bundle_uuid,
                req.new_bundle_name,
            )
        ...
        Some(rename_measurement_bundle_request::Selector::BundleName(bundle_name)) => {
            nico_api_db::measured_boot::bundle::rename_for_name(
                &mut txn,
                bundle_name,
                req.new_bundle_name,
            )
...
pub async fn handle_update_measurement_bundle(
    ...
    let bundle_id = match req.selector {
        ...
        Some(update_measurement_bundle_request::Selector::BundleName(bundle_name)) => {
            nico_api_db::measured_boot::bundle::from_name(&mut txn, bundle_name)
                .await
                .map_err(|e| CarbideError::Internal {
                    message: format!("deletion failed: {e}"),
                })?
    ...
    let bundle =
        nico_api_db::measured_boot::bundle::set_state_for_id(&mut txn, bundle_id, state.into())
            .await
            .map_err(|e| CarbideError::Internal {
...
pub async fn handle_show_measurement_bundle(
    ...
    let bundle = match req.selector {
        Some(show_measurement_bundle_request::Selector::BundleId(bundle_uuid)) => {
            nico_api_db::measured_boot::bundle::from_id(&mut txn, bundle_uuid)
                .await
                .map_err(|e| CarbideError::Internal {
                    message: format!("{e}"),
                })?
...
pub async fn handle_show_measurement_bundles(
    ...
    Ok(ShowMeasurementBundlesResponse {
        bundles: nico_api_db::measured_boot::bundle::get_all(&mut api.db_reader())
            .await
            .map_err(|e| CarbideError::Internal {
                message: format!("{e}"),
            })?
...
pub async fn handle_find_closest_match(
    ...
    let report = nico_api_db::measured_boot::report::from_id(&mut txn, report_id)
        .await
        .map_err(|e| CarbideError::Internal {
            message: format!("{e}"),
        })?;
    ...
    // get profile
    let journal =
        nico_api_db::measured_boot::journal::get_journal_for_report_id(&mut txn, report_id).await?;
