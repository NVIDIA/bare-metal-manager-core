...
pub async fn persist(
    ...
    db::network_prefix::create_for(txn, &segment_id, &value.prefixes).await?;
    db::network_segment_state_history::persist(txn, segment_id, &initial_state, version).await?;
...
pub async fn try_update_controller_state(
    ...
    match query_result {
        Ok(_segment_id) => {
            db::network_segment_state_history::persist(
                &mut *txn,
                segment_id,
                new_state,
                next_version,
...
pub async fn final_delete(
    ...
    db::network_prefix::delete_for_segment(segment_id, txn).await?;
...
pub async fn allocate_svi_ip(
    ...
    let (prefix_id, svi_ip) = if ipv4_prefix.num_reserved < 3 {
        ...
        if !value.segment_type.is_tenant() {
            db::machine_interface::allocate_svi_ip(txn, value).await?
        } else {
            db::instance_address::allocate_svi_ip(txn, value).await?
        }
    } else {
    ...
    db::network_prefix::set_svi_ip(txn, prefix_id, &svi_ip).await?;
