---
tags: ["current", "deployment", "ntp", "security", "database"]
---
...
1. Run `setup.sh` as normal to provision the new database and apply the ESO secret sync.
1. (Optional) Migrate data from the backup into `nico_rest` on `nico-pg-cluster` if you need to preserve existing REST state.
1. Confirm `nico-rest-pg-creds` exists in the `nico-rest` namespace before you start the `nico-rest-db` migration job
</Warning>
