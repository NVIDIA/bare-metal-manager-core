...
template <class T>
class CDHCPOptionsHandler;

// Base class for Option handling. 
template <class T>
class CDHCPOptionsManager {
    protected: 
...
// Currently only Option and OptionUint16 are supported as only these are used in pkt handler function.

template <>
class CDHCPOptionsHandler<OptionUint16>: public CDHCPOptionsManager<OptionUint16> {
    public:
        ...
        void resetAndAddOption(boost::any param) {
            option_val.reset(new OptionInt<uint16_t>(Option::V4, option, boost::any_cast<int>(param)));
        }
};
