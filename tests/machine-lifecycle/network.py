...
def wait_for_host_port(hostname: str, port: int, max_retries: int = 20, sleep_time: float = 30) -> None:
    ...
    """
    print(f"{datetime.datetime.now(datetime.UTC)}: Attempting to connect to {hostname} port {port} up to {max_retries} times with {sleep_time=}")
    retry_count = 0
    while retry_count < max_retries:
        ...
        raise TimeoutError(f"{datetime.datetime.now(datetime.UTC)}: Unable to contact {hostname} port {port}")
