...
class DellFactoryResetMethods:
    ...
    def reset_bios(self):
        ...
        if response.status_code == 200:
            print(
                "\n- PASS: status code %s returned for POST command to reset BIOS to default settings"
    ...
    def get_server_status(self):
        for _ in range(10):
            try:
                ...
                print(
                    "Error: Failed to decode JSON response from Redfish API. Retrying in 5 seconds..."
                )
                time.sleep(5)
        return data
    ...
    def reboot_server(self):
        ...
        if data["PowerState"] == "On":
            ...
            if response.status_code == 204:
                print(
                    "- PASS, POST command passed to gracefully power OFF server, status code return is %s"
                    % response.status_code
                )
                print(
            ...
            while True:
                ...
                if data["PowerState"] == "Off":
                    ...
                    if response.status_code == 204:
                        print(
                            "- PASS, POST command passed to perform forced shutdown, status code return is %s"
                            % response.status_code
                        )
                        time.sleep(15)
                        response = requests.get(
            ...
            print(
                "- FAIL, unable to get current server power state to perform either reboot or power on"
            )
            sys.exit(1)
