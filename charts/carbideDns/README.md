...
|-----|------|---------|-------------|
| carbideApiUrl | string | `"-"` | When set to "-" Helm will try to determine the carbide-api service name |
...
| container.command[2] | string | `"/opt/carbide/carbide-dns run --carbide-url=\"${CARBIDE_API}\""` |  |
| container.containerPorts.tcp | int | `1053` |  |
| container.containerPorts.udp | int | `1053` |  |
...
| container.nodeAffinityPreset.type | string | `""` | Node carbideDns.affinity preset ref: https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#node-affinity |
...
| fullnameOverride | string | `"carbide-dns"` | String to fully override common.names.fullname |
...
| ingress.extraHosts | list | `[]` | An array with additional hostname(s) to be covered with the ingress record e.g: extraHosts:   - name: carbide-dns.local     path: / |
...
| ingress.extraTls | list | `[]` | TLS configuration for additional hostname(s) to be covered with this ingress record ref: https://kubernetes.io/docs/concepts/services-networking/ingress/#tls e.g: extraTls: - hosts:     - carbide-dns.local   secretName: carbide-dns.local-tls |
...
| ingress.secrets | list | `[]` | Custom TLS certificates as secrets NOTE: 'key' and 'certificate' are expected in PEM format NOTE: 'name' should line up with a 'secretName' set further up If it is not set and you're using cert-manager, this is unneeded, as it will create a secret for you with valid certificates If it is not set and you're NOT using cert-manager either, self-signed certificates will be created valid for 365 days It is also possible to create and manage the certificates outside of this helm chart Please see README.md for more information e.g: secrets:   - name: carbide-dns.local-tls     key: |-       -----BEGIN RSA PRIVATE KEY-----       ...       -----END RSA PRIVATE KEY-----     certificate: |-       -----BEGIN CERTIFICATE-----       ...       -----END CERTIFICATE----- |
...
| nameOverride | string | `"carbide-dns"` | String to partially override common.names.name |
...
| volumePermissions.image.repository | string | `"nvidia/nvforge/bitnami-shell"` | Bitnami Shell image repository |
