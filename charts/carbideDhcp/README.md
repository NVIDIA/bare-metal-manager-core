...
| configDirPath | string | `"/tmp"` | Directory containing kea config file |
| container.affinity | object | `{}` |  |
| container.args[0] | string | `"-c"` |  |
| container.args[1] | string | `"/tmp/kea_config.json"` |  |
...
| container.command[0] | string | `"/sbin/kea-dhcp4"` |  |
| container.containerPorts.udp | int | `67` |  |
| container.containerSecurityContext.capabilities.add[0] | string | `"NET_ADMIN"` |  |
| container.containerSecurityContext.enabled | bool | `true` | Configure Container Security Context ref: https://kubernetes.io/docs/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-container |
| container.containerSecurityContext.readOnlyRootFilesystem | bool | `false` |  |
| container.containerSecurityContext.runAsNonRoot | bool | `false` |  |
| container.containerSecurityContext.runAsUser | int | `0` |  |
...
| container.livenessProbe.enabled | bool | `false` | Configure extra options for carbide containers' liveness and readiness probes ref: https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/#configure-probes |
| container.livenessProbe.failureThreshold | int | `5` |  |
| container.livenessProbe.initialDelaySeconds | int | `20` |  |
| container.livenessProbe.periodSeconds | int | `10` |  |
| container.livenessProbe.successThreshold | int | `3` |  |
| container.livenessProbe.timeoutSeconds | int | `5` |  |
| container.nodeAffinityPreset.key | string | `""` |  |
| container.nodeAffinityPreset.type | string | `""` | Node carbideDhcp.affinity preset ref: https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#node-affinity |
...
| fullnameOverride | string | `"carbide-dhcp"` | String to fully override common.names.fullname |
...
| ingress.extraHosts | list | `[]` | An array with additional hostname(s) to be covered with the ingress record e.g: extraHosts:   - name: carbide-dhcp.local     path: / |
...
| ingress.extraTls | list | `[]` | TLS configuration for additional hostname(s) to be covered with this ingress record ref: https://kubernetes.io/docs/concepts/services-networking/ingress/#tls e.g: extraTls: - hosts:     - carbide-dhcp.local   secretName: carbide-dhcp.local-tls |
...
| ingress.secrets | list | `[]` | Custom TLS certificates as secrets NOTE: 'key' and 'certificate' are expected in PEM format NOTE: 'name' should line up with a 'secretName' set further up If it is not set and you're using cert-manager, this is unneeded, as it will create a secret for you with valid certificates If it is not set and you're NOT using cert-manager either, self-signed certificates will be created valid for 365 days It is also possible to create and manage the certificates outside of this helm chart Please see README.md for more information e.g: secrets:   - name: carbide-dhcp.local-tls     key: |-       -----BEGIN RSA PRIVATE KEY-----       ...       -----END RSA PRIVATE KEY-----     certificate: |-       -----BEGIN CERTIFICATE-----       ...       -----END CERTIFICATE----- |
...
| loglevel | string | `"INFO"` | loglevel of KEA |
| nameOverride | string | `"carbide-dhcp"` | String to partially override common.names.name |
...
| service.clusterIP | string | `"10.96.0.67"` | carbide-dhcp service Cluster IP e.g.: clusterIP: None |
...
| service.nodePorts | object | `{"udp":67}` | Node ports to expose NOTE: choose port between <30000-32767> |
| service.nodePorts.udp | int | `67` | Node port for DHCP |
| service.ports.udp | int | `67` | carbide service DHCP port |
