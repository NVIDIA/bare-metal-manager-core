...
`e` as `error = %e`. On event payloads, do not reuse framework or formatter metadata keys such as `level`, `msg`,
`location`, `component`, `span_id`, or `span_name`; choose a domain-specific key such as `configured_log_level` or
`error_location` instead. Put component or correlation context on an enclosing span when appropriate. `event_name`
and `metric_name` are reserved for fields generated by `carbide_instrument::Event` and must not be authored on plain
events or Event payloads. Existing metric-only labels with these names remain allowed because renaming them would
change the metric contract.
...
`cargo make check-structured-logs` applies the high-confidence part of this contract to new production tracing calls.
It rejects reserved-key collisions, invalid field names, runtime-selected or interpolated messages, missing messages,
unambiguous aliases, and keys that collide after logfmt rendering. Existing exceptions live in a count-based baseline
that ignores line movement but fails when a violation is copied or an obsolete exception remains. Use
`cargo xtask inventory-structured-logs --format markdown` or `--format json` when reviewing the complete advisory
inventory; an unknown, well-formed domain field remains valid.
