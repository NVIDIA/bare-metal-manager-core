# Forge DPU configuration

Forge is NVIDIAs Bare-Metal-As-A-Service (BMaaS) solution. It manages the lifecycle of hosts including user OS installation, host cleanup, validation tests, and automated software updates. It also provides host monitoring, and provides virtualized private networking capabilities for Forge users (tenants) on ethernet and InfiniBand.
...
This document describes how Forge controls DPUs in order to achieve this behavior.
...
- The DPUs configuration can be reconstructed at any point in time. This is e.g. required after Forge installs a firmware update and new operating system on the DPU.
...
Forge uses versioned immutable configuration data in order to detect whether any intended changes have not yet been deployed:
- Every time a configuration for the DPU changes, an associated version number is increased.
- The version number is sent back from the DPU to the site controller as part of the `RecordDpuNetworkStatus`.
- If the reported version number of the DPU does match the last desired version number and if the DPU reports itself as healthy/operational, the Forge control plane knows that the configuration was deployed, and can report that fact to tenants. If the version number does not match the desired version number, or if the DPU is not yet healthy, the Instance will show up as `Provisioning`/`Configuring`/`Terminating` to tenants and administrators.
- **Forge will never show a configuration as applied without feedback from the DPU. Doing so would cause reliability issues (e.g. double-assignment of IPs) as well as cause security concerns**.
...
The DPU configuration that is applied can be understood as coming from 2 different sources:
  ...
  2. Forge site controller and Host lifecycle: During the lifecycle of a Host, certain parts of the network configuration need to be updated. E.g. when the Host is provisioned for a tenant, the Host networking gets reconfigured from using the admin overlay network towards the tenant overlay network. When the Host is released by the tenant, it is moved back onto the admin network.

In order to separate these concerns, Forge internally uses 2 different configuration data structs and associated version numbers (`instance_network_config` vs `managedhost_network_config`). It can thereby distinguish whether a setting that is required by the tenant has not been applied, compared to whether a setting that is required by the control plane has not been applied.

Some example workflows which lead to updating configurations are shown in the following diagram:

```mermaid
sequenceDiagram
    actor Tenant as Forge Tenant
...
One important requirement for Forge is that Hosts/DPUs that are not confirmed to be part of the site are isolated from the remaining hosts on the site.

A DPU might get isolated from the cluster without the DPU software stack being erased - e.g. by Forge site operators removing the knowledge of the DPU from the site database.
...
*Note: This is not the only mechanism that Forge utilizes to provide security on the networking layer. In addition to this, ACLs and routing table separation are used to implement secure virtual private networks (VPCs).*

## History

During the early days of Forge, DPU configuration had been applied by an external service called Hydrazine (or `Vpc`). Hydrazine was implemented as a kubernetes controller, which reconfigured DPUs based on requirements in Custom Resource Definitions (CRDs). Those CRDs had been updated by the Forge Site Controller (`carbide`). While this approach worked well enough to support the first limited releases of Forge, it was eventually replaced by the `forge-dpu-agent` based mechanism for the following reasons:
- The dpu-agent implements a bigger set of capabilities. Besides configuration updates, it also supports agent-updates, health monitoring and a set of additional features.
- Kubernetes CRDs are a stateful resource. Using CRDs required the Forge site controller to sync it's configuration to CRDs, and the Hydrazine controller to sync the CRD configs to the DPUs. The multi-level sync workflow was never fully implemented, and therefore there existed a possibility that DPUs would run with unintended configurations. Utilizing a stateless single-hop configuration workflow was a lower complexity and low effort solution to achieve reliable DPU configuration than to implement additional reconciliation workflows.
