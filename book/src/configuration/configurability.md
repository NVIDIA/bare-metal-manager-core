...
| `api_admission_control` | enabled; global `64` executing / `1024` pending; per client `8` executing / `64` pending; `5s` pending timeout; `5m` idle cleanup; no service overrides | Fairly schedules bounded per-client queues within the shared gRPC and admin business-request budget. Tune the global and per-client limits after scale testing; use exact SPIFFE service overrides for trusted internal services, and disable only as a rollback escape hatch. |
