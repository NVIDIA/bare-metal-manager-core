...
| `bmc_proxy` | unset | Routes nico-api's own BMC Redfish traffic through nico-bmc-proxy (`enabled`, `address`, SPIFFE cert paths), including established-endpoint credentialed exploration. Credential setup and rotation, session minting, exploration's anonymous vendor probes, and component-manager compute-tray power control (the `core` backend and standalone servers, which authenticate with explicit per-endpoint credentials) always stay direct. Proxied traffic requires the BMC's Redfish port to be 443; a BMC recorded on another port fails with a client-creation error naming the port. |
