...
| `bmc_proxy` | unset | Configure this when your deployment must route eligible `nico-api` Redfish requests through `nico-bmc-proxy`. Review the [routing contract](https://github.com/dsx-ai-factory/infra-controller/blob/main/crates/api-core/src/cfg/README.md#bmcproxyconfig--bmc_proxy) for direct-path exceptions, TLS credentials, precedence, and port requirements. |
