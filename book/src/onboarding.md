...
3. Try to identify the state transitions for your subnet and machine on the [Forge dashboard](https://ngcobservability-grafana.thanos.nvidiangn.net/d/WzX_VErVk/argo-forge-sites?orgId=1&var-ClusterName=dev03. You need access to the DL `grafana-ngcobservability-viewer`. Please don’t edit/modify the dashboard at this point in time. If Grafana asks you whether to save anything press cancel. For other DLs (including edit permissions), please check the [Thanos: Grafana Access confluence page](https://confluence.nvidia.com/pages/viewpage.action?spaceKey=PLATFORM&title=Thanos%3A+Grafana+Access#Thanos:GrafanaAccess-8.NGCObservability). Grafana access is limited to certain VPN endpoints that are described on that page. If the VPN endpoint you use prevents you from getting access to Thanos you can either use a different endpoint or work with the Thanos team to allow your preferred VPN endpoint.
