...
### Added in v2025.12.05-rc2-0

- [MR-4939](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4939): Added the ability to build NVIDIA-compliant distroless containers.
- [FORGE-7282](https://jirasw.nvidia.com/browse/FORGE-7282), [MR-4959](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4959): Added the ability to access `GetAllExpectedMachinesLinked` endpoint from Site Agent.
- [FORGE-7464](https://jirasw.nvidia.com/browse/FORGE-7464), [MR-4936](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4936): Added the ability to create multiple instances in a single transactional batch-allocation request, where any failure causes the entire batch to fail.
- [MR-4916](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4916): Added support for GH200 and nvswitches through Redfish (libredfish v0.33.0).
- [MR-4935](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4935): Added the ability to limit statecontroller iteration tables to the last 10 entries.
- [FORGE-7235](https://jirasw.nvidia.com/browse/FORGE-7235), [MR-4903](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4903): Added the ability to perform attestation with NRAS through a standalone REST API client library.
...
- [MR-4942](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4942): Changed the behavior of CPLD upgrades for Dell XE9680s to include both MB and PDB types.
- [MR-4907](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4907): Changed the behavior of the `sku unassign` command to add a `--force` flag in forge-admin-cli for unassigning SKUs from machines stuck in a problematic state.

### Fixed in v2025.12.05-rc2-0

- [NVBugs-5697527](https://nvbugspro.nvidia.com/bug/5697527), [MR-4961](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4961): Fixed an issue where iPXE did not retry the EFI RNG when receiving a NOT_READY error code.
- [MR-4957](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4957): Fixed an issue where applying an NSG with no rules caused traffic to default to VPC isolation instead of denying all traffic.
- [MR-4926](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4926): Fixed an issue where a non-critical warning was logged as an error, wasting developer time during troubleshooting.
- [MR-4956](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4956): Fixed an issue where `PRIVILEGED_INTERNAL` was missing from the `from_str` implementation for routing profiles.
- [MR-4952](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4952): Fixed an issue where the MNNVL search filter for machines failed to match some GB200 machines with "Compute Tray" in the name due to exact string matching.
- [MR-4947](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4947): Fixed an issue where nvlink config updates were incorrectly skipped when comparing configs with different `gpu_configs` vector lengths.
- [NVBugs-5706894](https://nvbugspro.nvidia.com/bug/5706894), [MR-4941](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4941): Fixed an issue where an error was immediately returned when the nvlink config was not available during MNNVL machine discovery.
- [NVBugs-5705966](https://nvbugspro.nvidia.com/bug/5705966), [FORGE-7507](https://jirasw.nvidia.com/browse/FORGE-7507),[MR-4932](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4932): Fixed an issue where elektra-site-agent lacked permissions to call `GetAllExpectedMachines` and `GetAllSkuIds` on carbide-api.
- [NVBugs-5706626](https://nvbugspro.nvidia.com/bug/5706626), [MR-4938](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4938): Fixed an issue where nvidia-smi was failing due to arguments being sent with unexpected white space causing failed GPU enumeration.
- [MR-4934](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4934): Fixed an issue in carbide-web where the restart reminder would indicate that the "endpoint" requires a restart instead of indicating that the "host" requires a reboot.
- [MR-4933](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4933): Fixed an issue where nvue would complain when no public prefixes were specified for the traffic intercept peer prefix list in the FNN template.
...
- [CDEVS-1241](https://jirasw.nvidia.com/browse/CDEVS-1241), [CDEVS-1246](https://jirasw.nvidia.com/browse/CDEVS-1246), [MR-4911](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4911): Added GitHub actions support.
- [MR-4954](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4954): Changed the behavior of state handler logging to remove the `times_in_state_s` and `handler_latencies_us` fields from the log span, as these values are no longer available after refactoring to emit counters and histograms directly from the state handler task.
- [MR-4885](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4885): Internal change where forge-prefixed components (forge_host_support, forge_systemd, forge_uuid, forge_version) were refactored to use carbide prefix.
- [MR-4944](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4944): Internal change where the pre-merge dev-env test was refactored to build the bfb boot artifact during the test job instead of depending on a previous pipeline job.
- [MR-4937](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4937): Internal change where a lint was added to detect and warn when database transactions are held open during await operations for non-database work, preventing connection pool exhaustion from idle transactions.
...
- [FORGE-6256](https://jirasw.nvidia.com/browse/FORGE-6256), [MR-4752](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4752): Added the ability to configure MlxConfigProfiles within carbide-api.
- [MR-4748](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4748): Added the ability to emit a metric on the amount of open logging/tracing spans.
- [5517703](https://nvbugspro.nvidia.com/bug/5517703), [FORGE-6989](https://jirasw.nvidia.com/browse/FORGE-6989), [MR-4740](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4740): Added the ability to profile memory usage through a dedicated endpoint.
- [MR-4738](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4738): Added the ability for FNN-enabled sites to choose TOR EVPN peering when route-servers are disabled.
- [FORGE-7085](https://jirasw.nvidia.com/browse/FORGE-7085), [MR-4735](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4735): Added the ability to set the primary DPU and boot order via CLI command.
- [MR-4714](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4714): Added the ability to quickly navigate to the logs for a managed host and explored enpoint from carbide-web to Grafana.
- [FORGE-7068](https://jirasw.nvidia.com/browse/FORGE-7068), [MR-4704](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4704): Added the ability to support TPM modules in SKUs.
- [FORGE-7097](https://jirasw.nvidia.com/browse/FORGE-7097), [MR-4675](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4675): Added the ability to enable `simulation mode` for SKU validation that allows viewing validation logs for SKU mismatches without disrupting users.

### Changed

- [5552492](https://nvbugspro.nvidia.com/bug/5552492), [MR-4716](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4716): Changed the behavior of HostReprovision state to include retry counting and fail after 5 attempts.
- [MR-4721](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4721): Changed the behavior of OVS `ctl-pipe-size` configuration to be dynamically adjusted based on DOCA version.

### Fixed

- [FORGE-3754](https://jirasw.nvidia.com/browse/FORGE-3754), [FORGE-6583](https://jirasw.nvidia.com/browse/FORGE-6583), [MR-4630](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4630): Fixed an issue with the tenant release workflow by including a power cycle, unlocking hosts and configuring BIOS settings as well as boot order appropriately.
- [MR-4761](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4761): Fixed an issue where GB200s were stuck in `HostInitializing/WaitingForPlatformConfiguration` state due to a lockdown check before issuing `machine_setup`. GB200s don't support lockdown or checking its status, so the state machine would hang when attempting to verify lockdown status. The fix skips the lockdown check when Redfish returns `NotSupported`, allowing the state machine to continue properly.
- [MR-4757](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4757): Fixed an issue where Network Security Group (NSG)-related ACL policies were always inserted in FNN NVUE config regardless of whether an NSG was applied, causing VPC isolation to have lower precedence than intended when NSGs were not applied. The fix ensures ACL policies are only inserted when an NSG is actually applied, matching the behavior of the pre-FNN template and maintaining proper security isolation precedence.
- [MR-4746](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4746): Fixed an issue where older profiling configurations would break due to missing default values in the profiling config.
- [MR-4743](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4743): Fixed an issue where static/network routes for FNN were not properly configured when on the admin network.
- [5587889](https://nvbugspro.nvidia.com/bug/5587889), [MR-4731](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4731): Fixed an issue where vault delete credentials was setting empty string instead of properly deleting entries from vault.
- [5564604](https://nvbugspro.nvidia.com/bug/5564604), [FORGE-7144](https://jirasw.nvidia.com/browse/FORGE-7144), [MR-4717](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4717): Fixed an issue where Vault's token refresh API was not idempotent, causing token invalidation when called multiple times in quick succession.
- [MR-4725](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4725): Fixed an issue where the Debian-based Rust container image changed from Debian 12 (bookworm) to Debian 13 (trixie) between versions 1.88 and 1.90, causing build artifacts linked against the libc6 version in trixie to be incompatible with the DPU BFB and resulting in linker errors when running the agent binary on the DPU.
- [5593406](https://nvbugspro.nvidia.com/bug/5593406), [MR-4759](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4759): Fixed an issue where iPXE would wait for input if downloading the iPXE script failed.
...
- [MR-4751](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4751): Internal change where robustness is added to MLT factory-reset password check to handle DPU instability by requiring 3 consecutive successful authentications with the default password before proceeding to force-delete, and moving the check closer to the force-delete step after host reset.
- [MR-4741](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4741): Internal change where a transaction wrapper is set in set_primary_dpu function.
- [MR-4739](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4739): Internal change where site_explorer no longer clones DPU reports.
- [MR-4744](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4744): Internal change where a re-assign step is added to the Machine Lifecycle Test script to reassign the instance-type in the event of a failure, providing a best-effort guard against team members inadvertently taking the machine between runs.
- [MR-4734](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4734): Internal change where Site Explorer clones are reduced to improve memory and CPU performance.
- [MR-4732](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4732): Internal change where `discover_machine` and `discovery_completed` functions were moved from the API module to handlers to reduce the size of the API module.
- [MR-4727](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4727): Internal change where a database transaction wrapper was introduced to reduce code duplication and enable unified transaction tracking and measurements.
- [MR-4724](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4724): Internal change where admin CLI options tests are moved to the `cfg::cli_options` module.
- [MR-4723](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4723): Internal change where a misnamed variable was fixed in MLT.
- [MR-4720](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4720): Internal change where the state handler outcome is refactored to use the `track_caller` feature instead of macros, replacing `SourceReference` with `std::panic::Location` for better error tracking and removing macro dependencies.
- [MR-4763](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4763): Internal change where code generation in tonic-client-wrapper and mlxconfig-registry was updated to avoid unnecessary rebuilds by only writing generated files when their content changes, and where tonic-client-wrapper now writes convenience converters in a sorted order to prevent random file changes between builds.
- [MR-4758](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4758): Internal change where unused imports error was fixed when building carbide-api without the linux-build flag by putting the import behind the same feature flag.
- [MR-4754](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4754): Internal change where some user-facing occurences for Forge & NGC were removed from the repo.
- [FORGE-7217](https://jirasw.nvidia.com/browse/FORGE-7217), [MR-4736](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4736): Internal change where a uniform naming convention is adopted for crates in preparation for moving them to a `crates/` directory. This includes dropping all "forge" naming in favor of "carbide", prefixing project-specific crates with `carbide-`, and only applying such prefixes to crates unique to this repo. Non-binary crates are also given a "0.0.0" version number to indicate they are not intended for publication. For now, `[lib]` names are unchanged to avoid source changes; only crate names are updated.
- [FORGE-7078](https://jirasw.nvidia.com/browse/FORGE-7078), [MR-4733](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4733): Internal change where a script was added to copy the carbide repo while sanitizing the result, enabling sharing of the codebase with Mirantis under a no-support agreement as a short-term measure prior to any open-sourcing efforts.
- [MR-4718](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4718): Internal change where carbide-api's integration tests were moved into their own crate to avoid circular dev-dependencies and unnecessary rebuilds, and where prometheus_text_parser was split into its own crate as a shared utility for carbide-api unit tests and ssh-console integration tests.
- [MR-4715](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4715): Internal change where the `carbide_api::db` module was split into its own crate (`carbide-api-db`). This involved moving the source and migration files, updating import statements, exposing some previously crate-private items as public, moving some test support code to parent modules, and relocating migration logic from `api/build.rs` to `api/db/build.rs`.
- [MR-4708](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4708): Internal change where `CURRENT_STATE_MODEL_VERSION` was moved out of the db crate, making it a parameter to relevant functions so that the `api::db` crate no longer depends on state machine code.
- [MR-4710](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4710): Internal change where types from machine_update_module were moved to the model crate to enable usage by the api::db crate as part of the carbide-api split.
- [MR-4705](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4705): Internal change where all usages of CarbideError were moved out of the db module as part of the carbide-api split, making `api::db` independent of CarbideError by introducing a DatabaseError enum that wraps either a SqlxError (`DatabaseError::AnnotatedSqlxError`) or variants that subsume CarbideError usages such as InvalidArgument and NotFound.
- [MR-4706](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4706): Internal change where `api::db` is refactored to remove its dependency on `api::Api` as part of the crate split, replacing usages of the Api object in database functions with more fine-grained alternatives such as directly calling `db::resource_pool::allocate` within `db::vpc::allocate_dpa_vni` and passing a transaction instead of an Api object when only a transaction is needed.
- [MR-4709](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4709): Internal change where IpAllocator types were moved to the db crate as part of the carbide-api split to eliminate a circular dependency.
- [MR-4711](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4711): Internal change where all database-related code is moved from measured_boot into the db crate as part of the carbide-api split.
- [MR-4707](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4707): Internal change where `api::db` is refactored to remove its dependency on CarbideConfig as part of the carbide-api split. This is accomplished by passing Firmware models directly to `snapshot_desired_firmware`, a `max_partition_per_tenant` field to `ib_partition::create`, and `HostHealthConfig` to `dpu_machine_updates::get_updated_machines` rather than passing the whole CarbideConfig.
...
- [MR-4680](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4680): Added the ability to automatically set a large ovs ctl-pipe-size when ovs-switchd starts, if not already set.
- [FORGE-7048](https://jirasw.nvidia.com/browse/FORGE-7048), [MR-4674](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4674): Added the ability for the DHCP server on the DPU to include the MTU size in its response.
- [FORGE-6739](https://jirasw.nvidia.com/browse/FORGE-6739), [MR-4642](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4642): Added the ability to pause and resume remediation actions for uningested endpoints via the UI, CLI, and API.
- [FORGE-6354](https://jirasw.nvidia.com/browse/FORGE-6354), [MR-4679](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4679): Added the ability to enable, disable, and check lockdown on a managed host via both the `forge-admin-cli` and carbide-web (including support for lockdown status queries, applying changes with optional reboot, and dynamic UI/button updates), as well as added the ability to view an explored endpoint page with a refreshed design, improved usability (tabbed sections, card layouts, disappearing error row), and a report age indicator showing the freshness of report data.
- [FORGE-7062](https://jirasw.nvidia.com/browse/FORGE-7062), [MR-4665](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4665): Added the ability to ensure a file is present or absent (delete files) using `duppet` by introducing a new `FileEnsure` capability, and updated the builder interface with `.with_content(..)`, `.with_perms(..)`, `.with_ownership(..)`, and `.with_ensure(..)` methods.
- [FORGE-2316](https://jirasw.nvidia.com/browse/FORGE-2316), [MR-4658](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4658): Added the ability to delete a network segment via `forge-admin-cli`, flagged under `unsafe cloud op`.
- [FORGE-7098](https://jirasw.nvidia.com/browse/FORGE-7098), [MR-4676](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4676): Added the ability to expose health and readiness endpoints using a common monitoring library, enabling direct integration with service liveness and readiness probes (such as Kubernetes). Currently, the DHCP server uses this common library to provide /health and /ready endpoints; future work will extend this to additional services.
- [FORGE-6887](https://jirasw.nvidia.com/browse/FORGE-6887), [MR-4523](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4523): Added the ability to use unique IDs for records in ExpectedMachines instead of the MAC address.

### Changed

- [MR-4648](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4648): Changed the behavior of the version command (`forge-admin-cli`) and configuration page (carbide-web) to display the DPU secure boot enabled flag.
- [FORGE-6506](https://jirasw.nvidia.com/browse/FORGE-6506), [MR-4667](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4667): Changed the behavior of the AdminPowerControl command to warn against taking action when the expected power state is Off and the desired state is On.

### Fixed

- [FORGE-7144](https://jirasw.nvidia.com/browse/FORGE-7144), [MR-4699](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4699): Fixed an issue where errors from the secret engine (Vault) were not specific, by introducing dedicated error types for missing credentials and improving error distinction. This addresses cases where missing credentials could previously be masked by generic errors.
- [5533959](https://nvbugspro.nvidia.com/bug/5533959) [5563186](https://nvbugspro.nvidia.com/bug/5563186), [MR-4641](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4641): Fixed an issue where qcow-imager could not find LVM devices for the root filesystem by adding an lvscan before device detection, and fixed an issue where grub updates could not be controlled by introducing optional command line arguments (`update_grub_template` and `update_grub_cfg`, defaulting to "yes") to make grub updates optional.
- [MR-4695](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4695): Fixed an issue where the host memory column always showed 0 GiB because the parser failed to recognize memory units when extra breakdown text was present (such as "1 TiB (64 GiBx16)"). The parsing logic now only considers the first two tokens (size and unit) so that trailing breakdown details do not interfere with the unit check.
- [5562541](https://nvbugspro.nvidia.com/bug/5562541), [MR-4684](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4684): Fixed an issue where `host_power_control` could fail on SMCs in a partial lockdown state when attempting to set `BootOnce(PXE)`. Previously, the code only attempted to unlock the SMC if lockdown was fully enabled, causing the unlock step to be skipped for partially locked SMCs (when the host interface was still enabled), which resulted in a failed boot override operation. The logic now correctly unlocks the SMC for any case that is not fully unlocked, ensuring host power control actions succeed.
- [5558047](https://nvbugspro.nvidia.com/bug/5558047), [MR-4677](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4677): Fixed an issue where the Power Manager continued increasing the retry counter even after reaching the maximum number of retries (3).
- [MR-4671](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4671): Fixed an issue where deserialization of `dpu_config` would overwrite default values instead of merging them, thereby not preserving default values that were absent in the passed config.
- [FORGE-7068](https://jirasw.nvidia.com/browse/FORGE-7068), [MR-4659](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4659): Fixed an issue where the TPM description was not properly added to the topology table and embedded nulls were present.
- [5548285](https://nvbugspro.nvidia.com/bug/5548285), [MR-4662](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4662): Fixed an issue where two DHCP clients (`dhclient` and `systemd-networkd`) would both manage the `oob_net0` interface on DPUs, sometimes resulting in conflicting leases or duplicate IP addresses. This was caused by an outdated workaround in `rc.local` that unnecessarily ran `dhclient`. The workaround has now been removed to prevent dual DHCP management and associated network inconsistencies.
- [MR-4685](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4685): Fixed an issue where errors occurring in `/admin/auth-callback` were not logged and only resulted in a generic HTTP 500 status in the logs. Errors are now logged with a custom error type for easier troubleshooting.
- [FORGE-6996](https://jirasw.nvidia.com/browse/FORGE-6996), [MR-4661](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4661): Fixed an issue where shutting down kea using `std::process::exit` would immediately terminate the process without running destructors, cleaning up FFI state, or allowing Rust objects to be dropped, potentially resulting in memory corruption, segmentation faults, or undefined behavior. The shutdown method now sends a termination signal to itself, allowing for proper and clean process termination.

### Internal Changes

- [MR-4660](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4660): Internal change where unit tests were added for redfish actions and race conditions were fixed to prevent duplicate approvers or duplicate action application.
- [MR-4702](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4702): Internal change where nightly format issues were fixed for a test file.
- [MR-4694](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4694): Internal change where documentation was added to the book about managing Azure storage database backup accounts and storage accounts in general.
- [MR-4647](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4647): Internal change where support was added for running the machine lifecycle test in the canary environment (site reno-qa3), logging output was improved, and test config variables were reworked (requiring manual updates to existing pipelines).
- [MR-4682](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4682): Internal change where documentation was added to the book explaining how to work with wal-g and Azure backups, including low-level functionality, manual operations, and troubleshooting steps.
- [MR-4681](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4681): Internal change where a fix was made for a trunk compilation failure and relevant unit tests were updated.
- [MR-4678](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4678): Internal change where the `allocate_port_guid` function was moved out of the `db` module, as it does not perform any database access.
- [MR-4669](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4669): Internal change where documentation updates from a previously auto-merged MR (!4668) were applied to the SKU book.
- [MR-4663](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4663): Internal change where the install script was updated to only gracefully reboot bf2 BMCs, removing the additional graceful reboot step for bf3 BMCs during installation since bf3 is resilient and handles upgrades automatically when the host loses power.
- [FORGE-7026](https://jirasw.nvidia.com/browse/FORGE-7026), [MR-4666](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4666): Internal change where a step was added to the CI after_script to run `docker builder prune -a` after dev-env tests, ensuring that gitlab runners clean up leftover docker resources after every job.
- [FORGE-6978](https://jirasw.nvidia.com/browse/FORGE-6978), [MR-4668](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4668): Internal change where the SKU page of the book was updated with new commands and an example for updating existing SKUs.
- [MR-4686](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4686): Internal change where CarbideError was dropped from models, a new ModelError type was introduced for use in the api/model crate as part of splitting carbide_api::model into its own crate, and api/src/db was updated to use only DatabaseError.
- [MR-4698](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4698): Internal change where cargo fmt was configured to use deterministic import ordering (by setting group_imports=StdExternalCrate and imports_granularity=module), enabling consistent formatting of import statements and reducing merge conflicts and diff churn; this required using nightly cargo for the formatting tasks only, integrated via the Makefile and Dockerfile.
- [MR-4689](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4689): Internal change where parts of CarbideConfig (including all Firmware types, HostHealthConfig, and DpuModel) were moved into the model module to break circular dependencies as part of preparing carbide_api::model to become its own crate.
- [MR-4687](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4687): Internal change where the NextState code was moved out of models as part of splitting carbide_api::model into its own crate. The state controller states remain in the model module for serialization and communication purposes, but the business logic for state transitions (which depends on StateHandlerError and is specific to state handling) is now implemented via traits such as ManagedHostStateHelper and ReprovisionStateHelper within state_handler.rs. This allows state transition logic (e.g., all_dpus_in_sync, next_bfb_install_state) to remain with state controllers even after types have moved to the new crate. This was a preparatory refactor prior to the full api/model split.
- [MR-4690](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4690): Internal change where certain infiniband (IB) types were moved to the models module as part of the ongoing carbide-api split, allowing model code to use these types directly and breaking the dependency in preparation for extracting carbide_api::model into its own crate.
- [MR-4693](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4693): Internal change where plugins and exec were enabled in the skaffold config to address CI build failures caused by a missing ksops plugin. The carbide base module in forged now requires ksops for a secretGenerator, necessitating changes to skaffold to pass the correct options to kustomize for enabling plugins and exec.
- [MR-4683](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4683): Internal change where basic RBAC settings were added for the new RLA pod.
- [MR-4673](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4673): Internal change where all "model-like" types, including database objects, were moved out of the db module and into the model module in preparation for separating model and db into distinct crates. After this change, the db module contains only private types and bare functions that operate on models. This is the initial step in splitting the carbide-api crate into multiple smaller crates, which will eventually separate model, db, state controllers, site explorer, gRPC handlers, and web code into their own crates. As a result of Rust's orphaning rules, FromRow implementations and some PgJson types must now reside in the model module.
...
- [MR-4606](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4606): Added the ability for the DPA statemachine to communicate with an MQTT broker to configure DPA Agents.
...
- [MR-4623](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4623): Internal change where the timeout for unzip during firmware downgrade in MLT was increased.
...
- [MR-4538](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4538): Bump libredfish to 0.29.71 to pull in a fix to setting the boot order on Lenovo SR 675 V3s.
...
- [MR-4524](https://gitlab-master.nvidia.com/nvmetal/carbide/-/merge_requests/4524): Added Merge Request template.
