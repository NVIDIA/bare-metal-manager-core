...
docker build \
  ...
  --build-arg VERGEN_GIT_DESCRIBE="v0.1.0-g$(git rev-parse --short HEAD)" \
  -t "${NICO_IMAGE_REGISTRY}/rms-api:v0.1.0-g$(git rev-parse --short HEAD)" .
docker push "${NICO_IMAGE_REGISTRY}/rms-api:v0.1.0-g$(git rev-parse --short HEAD)"
export NICO_RMS_IMAGE_REPO="${NICO_IMAGE_REGISTRY}/rms-api"
export NICO_RMS_IMAGE_TAG="v0.1.0-g$(git rev-parse --short HEAD)"
```
...
| `NICO_RMS_IMAGE_TAG` | Unless RMS is skipped (`--skip-rms` / `NICO_SKIP_RMS=true`) | RMS API server image tag (git-describe style, e.g. `v0.8.0`). No default - the chart fails at render without one. See `setup.sh` header for the full `NICO_RMS_*` family (chart override, image repo, NGC key). The namespace is fixed to `rack-manager` - NICo Core dials the service by that name. |
