...
#   REGISTRY_PULL_SECRET   NVIDIA API key for nvcr.io. Only needed if the
#                          nvcr-carbide-dev pull secret does not already exist
#                          in the machine-a-tron namespace.
#   REGISTRY_PULL_USERNAME Username for the pull secret. Default: $oauthtoken
#   MAT_IMAGE_TAG          machine-a-tron image tag. Default: value in the
#                          values file.
...
PULL_SECRET_NAME="nvcr-carbide-dev"
...
[[ -n "$MAT_IMAGE_TAG" ]] || die "could not determine image tag from $VALUES_FILE (set MAT_IMAGE_TAG)"
...
info "image tag: ${MAT_IMAGE_TAG}   hosts: ${HOST_COUNT}   dpus/host: ${DPU_PER_HOST}"
...
if kubectl get secret "$PULL_SECRET_NAME" -n "$MAT_NAMESPACE" >/dev/null 2>&1; then
    ...
    kubectl create secret docker-registry "$PULL_SECRET_NAME" -n "$MAT_NAMESPACE" \
        --docker-server=nvcr.io --docker-username="$REGISTRY_PULL_USERNAME" \
...
info "values: tag=${MAT_IMAGE_TAG} hosts=${HOST_COUNT} dpus=${DPU_PER_HOST} oob=${OOB_DHCP_RELAY} admin=${ADMIN_DHCP_RELAY}"
