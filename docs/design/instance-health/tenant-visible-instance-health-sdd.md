# Tenant-Visible Instance Health: Software Design Document

## Revision History

| Version | Date | Modified by | Description |
| :-----: | :--: | :---------- | :---------- |
| 0.1 | July 9, 2026 | Sunil Kumar | Initial version |

## 1. Introduction

Machine health is currently visible to site administrators, but tenants do not have a tenant-safe way to determine whether the host backing an Instance has a health issue.

Tenants need this information for operational workflows such as evacuation, SLURM scheduling decisions, and Kubernetes node drain automation. Tools like NVSentinel also consume external node health.
...
Aggregate health
: Core-derived Machine health report after combining relevant health report sources

API
: Application Programming Interface

DPU
: Data Processing Unit, also referred to as a SmartNIC

gRPC
: Remote procedure call API used by NICo Core

IMDS
: Instance Metadata Service

Instance
: Tenant-facing allocation running on a Machine

Machine
: Bare-metal host managed by NICo

Machine health
: Site-admin-facing health report for a Machine

NICo
: NVIDIA Infra Controller, a bare-metal life-cycle management system. Also referred to as Bare Metal Manager (BMM)

REST
: HTTP API exposed by NICo REST

SDD
: Software Design Document

Site admin
: Operator or administrator with access to site-level Machine health and repair workflows

SLURM
: Workload manager commonly used for HPC scheduling

Tenant
: A NICo client, org, or account that provisions and manages bare-metal nodes through NICo APIs
...
The following diagram shows the tenant-visible health projection flow:
...
1. [Existing system and additive changes](#31-existing-system-and-additive-changes): Shows what each layer already has and what this design adds.
1. [Tenant health projection model](#32-tenant-health-projection-model): Defines the structured API contract exposed through Instance status.
1. [Visibility and filtering rules](#33-visibility-and-filtering-rules): Defines which Machine health alerts can be shown to tenants.
1. [State ownership, state derivation, and severity](#34-state-ownership-state-derivation-and-severity): Defines who updates tenant health state and how state and severity are computed.
1. [API exposure](#35-api-exposure): Defines gRPC, REST, and IMDS behavior.
1. [Security](#38-security-considerations) and [compatibility](#37-compatibility): Defines the tenant isolation boundary and backward compatibility expectations.
...
The following table summarizes existing behavior and additive changes by layer:
...
Severity is owned by Core and derived internally from Machine health classifications, but the classifications themselves are not exposed. For the initial implementation, probes should not set a separate tenant severity. For example:

* Classifications that prevent allocations, instance deletion, or host state changes can map to `Critical`.
* Tenant-visible alerts with no classification can map to `Info`.
* Other tenant-visible alerts can map to `Warning`.
