...
Indicates that a host/DPU could not establish a BGP session with a route server that is part of the NICo control plane.
Route servers are redundant. This alert only applies the `PreventAllocations` and `PreventHostStateChanges` classifications when there is _at most_ one healthy session left (including a single configured route server going down). With more than one healthy session, this is just a warning. A session is healthy when its state is `Established`; each configured route server that is not gets its own alert.
