...
- **PostgreSQL** — stores all NICo system state in the `nico_system_nico` database. Only the API Service reads from and writes to it. The reference deployment uses CloudNativePG operator `1.30.0` with the PostgreSQL `15.18` standard image.
