...
| `ScoutReportPci` | `BOOT_INTERFACE_SELECTION_SOURCE_SCOUT_REPORT_PCI` | NICo used [`domain:bus:device.function` PCI addresses in a host scout report](https://github.com/NVIDIA/infra-controller/issues/5083) to confirm or change a selection previously recorded as `RedfishChassisId` or `RedfishSerialNumber`. |
