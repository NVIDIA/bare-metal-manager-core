...
| `dpu_service_sync_enabled` | bool | `true` | Whether NICo releases the DPF maintenance hold on a changed DPUService by itself, for hosts that are `Ready`, unassigned, and whose DPUs already match their `DPUDeployment`. Setting `false` does not resume unchecked rollout: the held DPUs wait for an operator to release them (refer to [DPUService rollout gating](#dpuservice-rollout-gating)). |
