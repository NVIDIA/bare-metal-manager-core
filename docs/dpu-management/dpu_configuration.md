...
- A systemd-managed agent invokes `lldpcli` directly. Provisioning renames
  `/etc/lldpd.d/lldp-interfaces.conf` to
  `/etc/lldpd.d/lldp-interfaces.conf.disabled`, configures
  `DAEMON_ARGS="-M 1"`, and restarts `lldpd` so LLDP-MED inventory is available
  on all interfaces.
- A DPF deployment runs `nico-lldp-sidecar` from the same image as `dpu-agent`.
  The sidecar invokes the host's `lldpcli`, writes an atomic snapshot to the
  shared `/data/lldp` path, and the containerized agent reads that snapshot.
...
For deployment settings and log collection, see
[DPF Setup for NICo Integration](../manuals/dpf.md#dpu-lldp-sidecar). For
operational checks, see [DPU Health Checks](../operations/monitoring-health.md#dpu-health-checks).
