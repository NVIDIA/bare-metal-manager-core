    ...
    T: UuidSubtype,
{
    ...
    // Construct a TypedUuid<T> from an Option<crate::Uuid>, which is a common
    // pattern since gRPC nested messages are always optional at the protocol
    // level, even if not explicitly marked as such.
    pub fn from_required_rpc_uuid_field(
        value: Option<crate::Uuid>,
        field_name: &'static str,
    ) -> Result<Self, RpcDataConversionError> {
        match value {
            Some(ref rpc_uuid) => Self::try_from(rpc_uuid).map_err(|_e| {
                RpcDataConversionError::InvalidUuid(T::TYPE_NAME, rpc_uuid.to_string())
            }),

            None => Err(RpcDataConversionError::MissingArgument(field_name)),
        }
    }
}
...
impl<T> TryFrom<&crate::Uuid> for TypedUuid<T>
where
    T: UuidSubtype,
{
    type Error = UuidError;

    fn try_from(rpc_uuid: &crate::Uuid) -> Result<Self, Self::Error> {
        let rpc_uuid = rpc_uuid.value.as_str();
        Self::try_parse(rpc_uuid)
    }
}
...
impl<T> From<TypedUuid<T>> for crate::Uuid
where
    T: UuidSubtype,
{
    fn from(value: TypedUuid<T>) -> Self {
        let uuid = value.uuid;
        crate::Uuid::from(uuid)
    }
}
