...
func (s *GetDpuMachinesTestSuite) Test_GetDpuMachines_Success() {
	...
	expectedResult := &cwssaws.DpuMachineList{Machines: []*cwssaws.DpuMachine{
		{
			...
			DpuNetworkConfig: &cwssaws.ManagedHostNetworkConfigResponse{
				VniDevice:    "vxlan48",
				IsPrimaryDpu: true,
				NetworkSecurityPolicyOverrides: []*cwssaws.ResolvedNetworkSecurityGroupRule{
					{
						Rule: &cwssaws.NetworkSecurityGroupRuleAttributes{
							SourceNet:      &cwssaws.NetworkSecurityGroupRuleAttributes_SrcPrefix{SrcPrefix: "10.0.0.0/24"},
							DestinationNet: &cwssaws.NetworkSecurityGroupRuleAttributes_DstPrefix{DstPrefix: "10.1.0.0/24"},
						},
					},
				},
			},
