...
1. Check prerequisites. Run:
   ...
   You need Go 1.26.5 or newer, plus working `git` and `make`. If any is missing or too old, stop and tell the user what to install. Do not attempt to install Go, git, or make yourself unless the user explicitly asks.
...
- **`command not found: go`** — Go is not installed. Stop and tell the user to install Go 1.26.5 or newer from <https://go.dev/dl/>. Do not auto-install Go.

- **`go: go.mod requires go >= 1.26.5`** — Installed Go is too old. Stop and ask the user to upgrade.
