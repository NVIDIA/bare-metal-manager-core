...
// sampleCampaignCreateRequest returns a fully-populated create request that
// exercises every branch of the configuration tree.
func sampleCampaignCreateRequest() APICampaignCreateRequest {
	ruleID := "rule-id"
	return APICampaignCreateRequest{
		SiteID:      "site-id",
		Name:        "fw-rollout",
		Description: "desc",
		Selector: &APICampaignSelector{
			Percentage: &APICampaignPercentageSelector{Percent: 50, Seed: "seed-1"},
		},
		Options: APICampaignOptions{
			MaxConcurrentTargets: 3,
			SafetyPolicy: &APICampaignSafetyPolicy{
				Gates: []APICampaignSafetyGate{
					{FailureRate: &APICampaignFailureRateGate{Scope: "currentPhase", ThresholdPercent: 20}},
					{FailureCount: &APICampaignFailureCountGate{Scope: "cumulativeRun", ThresholdCount: 5}},
				},
			},
			ConflictPolicy: &APICampaignConflictPolicy{
				Retry: &APICampaignConflictRetry{
					RetryTimeout:      "30m",
					InitialRetryDelay: "10s",
					MaxRetryDelay:     "5m",
				},
			},
			OrderingPolicy: &APICampaignOrderingPolicy{
				Random: &APICampaignRandomOrdering{Seed: "seed-2"},
			},
			PhasePolicy: &APICampaignPhasePolicy{
				Equal:       &APICampaignEqualPhases{PhaseCount: 4},
				AutoAdvance: true,
			},
		},
		Operation: APICampaignOperation{
			Firmware: &APICampaignFirmwareOperation{
				Version:                "1.2.3",
				RuleID:                 &ruleID,
				OverrideReadinessCheck: true,
				SubTargets:             []string{"bmc", "bios"},
			},
			ExcludeCampaignIDs: []string{"prev-1", "prev-2"},
		},
	}
}

func TestAPICampaignCreateRequest_Validate(t *testing.T) {
	tests := []struct {
		name    string
		mutate  func(r *APICampaignCreateRequest)
		wantErr string
	}{
		{name: "valid full", mutate: func(r *APICampaignCreateRequest) {}},
		{
			name: "valid minimal",
			mutate: func(r *APICampaignCreateRequest) {
				r.Selector = nil
				r.Options = APICampaignOptions{MaxConcurrentTargets: 1}
				r.Operation = APICampaignOperation{Firmware: &APICampaignFirmwareOperation{Version: "1.0.0"}}
			},
		},
		{
			name:    "missing siteId",
			mutate:  func(r *APICampaignCreateRequest) { r.SiteID = "" },
			wantErr: "siteId is required",
		},
		{
			name:    "missing name",
			mutate:  func(r *APICampaignCreateRequest) { r.Name = "" },
			wantErr: "name is required",
		},
		{
			name:    "non-positive maxConcurrentTargets",
			mutate:  func(r *APICampaignCreateRequest) { r.Options.MaxConcurrentTargets = 0 },
			wantErr: "maxConcurrentTargets must be greater than zero",
		},
		{
			name:    "missing firmware operation",
			mutate:  func(r *APICampaignCreateRequest) { r.Operation.Firmware = nil },
			wantErr: "operation.firmware is required",
		},
		{
			name:    "missing firmware version",
			mutate:  func(r *APICampaignCreateRequest) { r.Operation.Firmware.Version = "" },
			wantErr: "operation.firmware.version is required",
		},
		{
			name: "gate sets both",
			mutate: func(r *APICampaignCreateRequest) {
				r.Options.SafetyPolicy.Gates = []APICampaignSafetyGate{{
					FailureRate:  &APICampaignFailureRateGate{Scope: "currentPhase", ThresholdPercent: 10},
					FailureCount: &APICampaignFailureCountGate{Scope: "currentPhase", ThresholdCount: 1},
				}}
			},
		...
		},
		{
			...
			mutate: func(r *APICampaignCreateRequest) {
				r.Options.SafetyPolicy.Gates = []APICampaignSafetyGate{{
					FailureRate: &APICampaignFailureRateGate{Scope: "bogus", ThresholdPercent: 10},
				}}
			},
		...
		},
		{
			...
			mutate: func(r *APICampaignCreateRequest) {
				r.Options.PhasePolicy = &APICampaignPhasePolicy{
					Equal:      &APICampaignEqualPhases{PhaseCount: 2},
					Percentage: &APICampaignPercentagePhases{Phases: []int32{50, 50}},
				}
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := sampleCampaignCreateRequest()
			tt.mutate(&req)
			err := req.Validate()
			if tt.wantErr == "" {
...
func TestAPICampaignCreateRequest_ToProto(t *testing.T) {
	req := sampleCampaignCreateRequest()
...
func TestAPICampaignCreateRequest_ToProto_OmitsUnsetRuleID(t *testing.T) {
	req := sampleCampaignCreateRequest()
...
func TestAPICampaignCreateRequest_ToProto_InvalidDuration(t *testing.T) {
	req := sampleCampaignCreateRequest()
...
func TestAPICampaignCreateRequest_ToProto_PhasePercentageAndCount(t *testing.T) {
	req := sampleCampaignCreateRequest()

	req.Options.PhasePolicy = &APICampaignPhasePolicy{Percentage: &APICampaignPercentagePhases{Phases: []int32{30, 70}}}
	pb, err := req.ToProto()
	require.NoError(t, err)
	...
	req.Options.PhasePolicy = &APICampaignPhasePolicy{Count: &APICampaignCountPhases{Phases: []int32{2, 3}}}
	pb, err = req.ToProto()
	require.NoError(t, err)
...
func TestAPICampaign_FromProto(t *testing.T) {
	...
	got := NewAPICampaignFromProto(run)
...
func TestAPICampaign_FromProtoSummary_NoStats(t *testing.T) {
	got := NewAPICampaignFromSummary(&flowv1.OperationRunSummary{
		Id:    &flowv1.UUID{Id: "campaign-id"},
		Name:  "fw-rollout",
...
func TestAPICampaignTarget_FromProto(t *testing.T) {
	...
	got := NewAPICampaignTarget(target)
	assert.Equal(t, "target-id", got.ID)
	assert.Equal(t, "campaign-id", got.CampaignID)
	...
	assert.Equal(t, APICampaignTargetStatusSubmitted, got.Status)
	assert.Equal(t, "submitted", got.Message)
}

func TestAPICampaignTarget_FromProto_UnsetTaskID(t *testing.T) {
	got := NewAPICampaignTarget(&flowv1.OperationRunTarget{
		Id:     &flowv1.UUID{Id: "target-id"},
		TaskId: &flowv1.UUID{Id: ""},
		Status: flowv1.OperationRunTargetStatus_OPERATION_RUN_TARGET_STATUS_PENDING,
	})
	assert.Nil(t, got.TaskID)
	assert.Equal(t, APICampaignTargetStatusPending, got.Status)
}

func TestAPICampaignGetRequest_Validate(t *testing.T) {
	require.Error(t, (&APICampaignGetRequest{}).Validate())
	require.NoError(t, (&APICampaignGetRequest{SiteID: "site-id"}).Validate())
}

func TestAPICampaignSiteRequest_Validate(t *testing.T) {
	require.Error(t, (&APICampaignSiteRequest{}).Validate())
	require.NoError(t, (&APICampaignSiteRequest{SiteID: "site-id"}).Validate())
}

func TestAPICampaignAdvanceRequest_Validate(t *testing.T) {
	require.Error(t, (&APICampaignAdvanceRequest{}).Validate())
	require.NoError(t, (&APICampaignAdvanceRequest{SiteID: "site-id"}).Validate())
	require.NoError(t, (&APICampaignAdvanceRequest{SiteID: "site-id", ExpectedPhaseIndex: int32Ptr(2)}).Validate())
}

func TestAPICampaignCancelRequest_Validate(t *testing.T) {
	require.Error(t, (&APICampaignCancelRequest{}).Validate())
	require.NoError(t, (&APICampaignCancelRequest{SiteID: "site-id"}).Validate())
}

func TestAPICampaignListRequest_Validate(t *testing.T) {
	tests := []struct {
		name    string
		req     APICampaignListRequest
		wantErr string
	}{
		{name: "valid no filters", req: APICampaignListRequest{SiteID: "site-id"}},
		{
			name: "valid with filters",
			req:  APICampaignListRequest{SiteID: "site-id", Status: "Running", OperationType: APIOperationTypeFirmwareControl},
		},
		{name: "missing siteId", req: APICampaignListRequest{}, wantErr: "siteId"},
		{name: "invalid status", req: APICampaignListRequest{SiteID: "site-id", Status: "bogus"}, wantErr: "status must be one of"},
		{name: "invalid operationType", req: APICampaignListRequest{SiteID: "site-id", OperationType: "bogus"}, wantErr: "operationType must be one of"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := tt.req.Validate()
...
func TestAPICampaignListRequest_ToProto(t *testing.T) {
	pageNum, pageSize := 2, 10
	req := APICampaignListRequest{SiteID: "site-id", Status: "Paused", OperationType: APIOperationTypeFirmwareControl}
...
func TestAPICampaignListRequest_QueryValues(t *testing.T) {
...
func TestAPICampaignTargetsListRequest_Validate(t *testing.T) {
	tests := []struct {
		name    string
		req     APICampaignTargetsListRequest
		wantErr string
	}{
		{name: "valid no filters", req: APICampaignTargetsListRequest{SiteID: "site-id"}},
		{
			name: "valid with filters",
			req:  APICampaignTargetsListRequest{SiteID: "site-id", Status: APICampaignTargetStatusFailed, PhaseScope: "completedPhases"},
		},
		{name: "missing siteId", req: APICampaignTargetsListRequest{}, wantErr: "siteId"},
		{name: "invalid status", req: APICampaignTargetsListRequest{SiteID: "site-id", Status: "bogus"}, wantErr: "status must be one of"},
		{name: "invalid phaseScope", req: APICampaignTargetsListRequest{SiteID: "site-id", PhaseScope: "bogus"}, wantErr: "phaseScope must be one of"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := tt.req.Validate()
...
func TestAPICampaignTargetsListRequest_ToProto(t *testing.T) {
	pageNum, pageSize := 3, 20
	req := APICampaignTargetsListRequest{SiteID: "site-id", Status: APICampaignTargetStatusFailed, PhaseScope: "completedPhases"}
	page := pagination.PageRequest{PageNumber: &pageNum, PageSize: &pageSize}

	pb := req.ToProto("campaign-id", page)
	assert.Equal(t, "campaign-id", pb.GetOperationRunId().GetId())
...
func TestAPICampaignTargetsListRequest_ToProto_DefaultsPhaseScope(t *testing.T) {
	req := APICampaignTargetsListRequest{SiteID: "site-id"}
	pb := req.ToProto("campaign-id", pagination.PageRequest{})
