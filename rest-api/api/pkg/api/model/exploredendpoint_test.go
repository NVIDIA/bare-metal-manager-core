...
func TestNewAPIExploredEndpoint_FromProto(t *testing.T) {
	...
	assert.Equal(t, "On", got.Report.Systems[0].PowerState)
	require.NotNil(t, got.Report.Systems[0].Attributes)
	require.NotNil(t, got.Report.Systems[0].Attributes.NicMode)
	assert.Equal(t, "Dpu", *got.Report.Systems[0].Attributes.NicMode)
	...
	assert.Equal(t, "Partial", got.Report.LockdownStatus.Status)
