...
func (h DecommissionManagedHostHandler) Handle(c echo.Context) error {
	...
	apiErr := common.ExecuteCoreGRPC(ctx, stc, corev1.Forge_DecommissionManagedHost_FullMethodName, &corev1.MachineId{Id: machineID}, nil, machine.Site.ID.String())
	if apiErr != nil {
...
// DeleteDecommissionedManagedHostHandler permanently removes a decommissioned managed host.
type DeleteDecommissionedManagedHostHandler struct {
	dbSession  *cdb.Session
	scp        *sc.ClientPool
	tracerSpan *cutil.TracerSpan
}

// NewDeleteDecommissionedManagedHostHandler returns a new managed-host deletion handler.
func NewDeleteDecommissionedManagedHostHandler(dbSession *cdb.Session, scp *sc.ClientPool, _ *config.Config) DeleteDecommissionedManagedHostHandler {
	return DeleteDecommissionedManagedHostHandler{
		dbSession:  dbSession,
		scp:        scp,
		tracerSpan: cutil.NewTracerSpan(),
	}
...
// Handle godoc
// @Summary Delete a decommissioned managed host
// @Description Permanently remove a managed host after decommissioning completes.
// @Tags Machine
// @Produce json
// @Security ApiKeyAuth
// @Param org path string true "Name of NGC organization"
// @Param id path string true "ID of decommissioned managed host Machine"
// @Success 204
// @Router /v2/org/{org}/nico/machine/{machineId}/decommission [delete]
func (h DeleteDecommissionedManagedHostHandler) Handle(c echo.Context) error {
	org, dbUser, ctx, logger, handlerSpan := common.SetupHandler("Machine", "DeleteDecommissioned", c, h.tracerSpan)
	if handlerSpan != nil {
		defer handlerSpan.End()
	}
	...
	logger.Info().Str("machine_id", machineID).Str("site_id", machine.Site.ID.String()).Msg("Deleting decommissioned managed host via Core gRPC proxy")
	apiErr := common.ExecuteCoreGRPC(ctx, stc, corev1.Forge_DeleteDecommissionedManagedHost_FullMethodName, &corev1.DeleteDecommissionedManagedHostRequest{
		MachineId: &corev1.MachineId{Id: machineID},
	}, nil, machine.Site.ID.String())
	if apiErr != nil {
		logAPIError(logger, apiErr, "Failed to delete decommissioned managed host via Core gRPC proxy")
		return cutil.NewAPIErrorResponse(c, apiErr.Code, apiErr.Message, nil)
	}
