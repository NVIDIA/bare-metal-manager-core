...
func executeGRPCProxy(
	...
	if err := we.Get(wfCtx, &out); err != nil {
		...
		if errors.As(err, &timeoutErr) || errors.Is(err, context.DeadlineExceeded) || wfCtx.Err() != nil {
			return cutil.NewAPIError(http.StatusGatewayTimeout, fmt.Sprintf("%s proxy request timed out", backend.Label), fmt.Errorf("%s proxy %s timed out: %w", strings.ToLower(backend.Label), fullMethod, err))
		}
	...
	if resp != nil && len(out.ResponseJSON) > 0 {
		if err := (protojson.UnmarshalOptions{DiscardUnknown: true}).Unmarshal(out.ResponseJSON, resp); err != nil {
			return cutil.NewAPIError(http.StatusInternalServerError, fmt.Sprintf("Failed to decode %s proxy response", backend.Label), fmt.Errorf("decode response for %s: %w", fullMethod, err))
		}
	}
	return nil
}
