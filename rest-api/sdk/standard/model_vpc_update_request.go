...
type VpcUpdateRequest struct {
	...
	// External power provisioning resource group to associate with the VPC. A non-empty value requires the Site's `powerProvisioning` mode to be `external`. Omission or `null` preserves the current association; an empty string clears it even when power provisioning is disabled.
