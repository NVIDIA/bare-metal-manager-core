...
A non-empty `powerResourceGroup` requires the Site's `powerProvisioning` mode to be `external`; otherwise the request returns 412. Omission or `null` preserves the current association, while an empty string clears it even when power provisioning is disabled.
