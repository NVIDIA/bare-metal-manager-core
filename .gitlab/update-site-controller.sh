#!/bin/bash
# For use by pipeline job "scheduled-test:update-site-controller"
set -euo pipefail
...
function update_git() {
  ...
  echo "Waiting up to 30 secs for MR & CI to be ready..."
...
cd envs/"${SCHEDULED_TEST_SITE}"/site/site-controller
...
if [[ -n $git_status ]]; then
  ...
  if ! echo "${INITIAL_SYNC_STATUS}" | grep -q "OutOfSync"; then
    echo "${SCHEDULED_TEST_SITE} is already configured for the latest site-controller in forged and Argo CD is in-sync. Nothing to do."; exit 0
  fi
fi
sync_argocd
